...
> **本目录是 [Sliverkiss/workbuddy2api](https://github.com/Sliverkiss/workbuddy2api) 系网关在本仓库中的整合副本**（gateway/）。
> 完整插件（含 DSH 侧 xuedinerAPI provider：CodeArts / WorkBuddy 号池 / ZCode / Qoder / Codex）见仓库根 README。
> 上游设计的精巧之处（账号池调度、错误分类、提示词体系）原样保留。
...
```bash
# 1. 克隆
git clone https://github.com/xuediner-source/dsh-xuediner-gateway.git
cd dsh-xuediner-gateway/gateway
